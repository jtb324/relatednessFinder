...
class IncorrectGridFileFormat(Exception):
    ...
    def __init__(self, line_num: int, grid_file: str) -> None:
        super().__init__(f"There was an error reading in the file: {grid_file} at line {line_num}. Program expected each line to be a separate ID.")
...
def determine_combinations(id_list: list[str], logger: logging.Logger) -> dict[str, dict[str, int]]:
    ...
    for grid in id_list:
        return_dict.setdefault(grid, {ind:0 for ind in id_list if ind != grid and ind not in return_dict.keys()})
...
def get_relatedness(query: str, connection: sqlite3.Connection, combinations: dict[str, dict[str, int]], logger: logging.Logger) -> None:
...
def write_to_file(results: dict[str, dict[str, int]], output_filename: Path) -> None:
...
def main(
    ...
    output_path: Path = typer.Option(
        ...
        "-o", 
        "--output",
        help="Filepath to write the output to."
    ),
    ...
    # We are going to generate a dictionary that has all of the possible pairwise combinations
    ...
    sql_str = "SELECT * FROM " + table_name + f" WHERE ID1 in {grid_str}" + f" AND ID2 in {grid_str};" 
    ...
    write_to_file(combinations, output_path)
